...
public class DoorController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {//nesnenin pozisyonu theplayerın pozisyonuna göre < açılmas mesafesi ise
        ...
        if(playerExiting)
        {//eğer playerexiting true ise player exit pointe mvoeplayerspeed hızında ilerlicek
            thePlayer.transform.position = Vector2.MoveTowards(thePlayer.transform.position, exitPoint.transform.position, movePlayerSpeed * Time.deltaTime);
        }
    }
    ...
    IEnumerator UseDoorCo()
    {
        ...
        RespawnController.instance.SetSpawn(exitPoint.position);//sonraki sahnedeki doğacağımız yeri ayarladık
